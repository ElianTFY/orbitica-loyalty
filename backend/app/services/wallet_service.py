...
def _hex_color(value: str) -> str:
    value = (value or "#0EA5FF").lstrip("#")
    if len(value) != 6:
        return "rgb(14,165,255)"
    try:
        r = int(value[0:2], 16)
        g = int(value[2:4], 16)
        b = int(value[4:6], 16)
...
def apple_pkpass(business: Business, customer: Customer) -> bytes:
    if not settings.apple_wallet_configured:
        raise RuntimeError("Apple Wallet no est? configurado.")
    ...
    balance_str = f"{customer.stamp_balance}/{business.stamps_required}"
    if business.program_type == "points":
        balance_str = f"{customer.point_balance} pts"
    ...
    pass_json = {
        ...
        "storeCard": {
            ...
            "auxiliaryFields": [
                {"key": "code", "label": "C?DIGO", "value": customer.card_code}
            ],
            "backFields": [
                {
                    "key": "terms",
                    "label": "Programa",
                    "value": f"Acumul? en {business.name} y recib?: {business.reward_name}.",
                },
                {
                    "key": "web",
                    "label": "Tarjeta web",
...
def google_save_url(business: Business, customer: Customer) -> str:
    if not settings.google_wallet_configured:
        raise RuntimeError("Google Wallet no est? configurado.")
    ...
    loyalty_class = {
        ...
        "programName": f"{business.name} Loyalty",
    ...
    loyalty_object = {
        ...
        "loyaltyPoints": {
            "label": "Progreso" if business.program_type != "points" else "Puntos",
            "balance": {"int": customer.stamp_balance if business.program_type != "points" else customer.point_balance},
        },
        "barcode": {
            "type": "QR_CODE",
        ...
        "textModulesData": [
            {"id": "reward", "header": "Premio", "body": business.reward_name},
            {
                "id": "progress",
                "header": "Progreso",
                "body": f"{customer.stamp_balance}/{business.stamps_required} sellos" if business.program_type != "points" else f"{customer.point_balance} puntos",
            },
        ],
        "linksModuleData": {
            "uris": [
