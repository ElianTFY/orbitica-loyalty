...
from .wallet_service import google_service_account, google_wallet_ids
...
def _notification_copy(business: Business, customer: Customer, reason: str) -> tuple[str, str]:
    progress = f"{customer.stamp_balance}/{business.stamps_required} sellos" if business.program_type != "points" else f"{customer.point_balance} puntos"
    if reason == "reward":
        return (
            f"?? Premio disponible en {business.name}",
            f"Llegaste a {progress}. Ya pod?s canjear: {business.reward_name}.",
        )
    if reason == "redeem":
        return (
            f"Premio canjeado en {business.name}",
            f"Tu premio fue canjeado. Tu nuevo saldo es {progress}.",
        )
    if reason == "program":
        return (
            f"{business.name} actualiz? su programa",
            f"Tu tarjeta ahora requiere {business.stamps_required} sellos para: {business.reward_name}.",
        )
    return (
        f"Nuevo movimiento en {business.name}",
        f"Ahora ten?s {progress}. Premio: {business.reward_name}.",
    )
...
def _google_patch_payload(business: Business, customer: Customer, notify: bool) -> dict:
    balance_val = customer.stamp_balance if business.program_type != "points" else customer.point_balance
    progress_str = f"{customer.stamp_balance}/{business.stamps_required} sellos" if business.program_type != "points" else f"{customer.point_balance} puntos"
    payload = {
        "loyaltyPoints": {
            "label": "Progreso" if business.program_type != "points" else "Puntos",
...
def _apple_client():
    ...
    if not private_key or not cert:
        raise RuntimeError("Certificado Apple Wallet inv?lido.")
