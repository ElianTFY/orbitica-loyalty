...
from ...schemas.business import PublicBusinessOut
from ...schemas.customer import PublicCardOut, PublicJoinIn, PublicJoinOut
from ...schemas.wallet import PushSubscriptionIn, PushUnsubscribeIn
from ...services.customer_service import CustomerService
from ...services.wallet_service import apple_pkpass, google_save_url
...
def public_join(request: Request, slug: str, payload: PublicJoinIn, db: Session = Depends(get_db)):
    ...
    phone_key = privacy_key(payload.phone)
    return CustomerService.public_join(db, slug, payload, ip, phone_key)
...
def public_card(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    return PublicCardOut(
        business=PublicBusinessOut.model_validate(business),
...
def wallet_apple(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    if not settings.apple_wallet_configured:
        raise HTTPException(503, "Apple Wallet todav?a no est? configurado.")
    try:
...
def wallet_google(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    if not settings.google_wallet_configured:
        raise HTTPException(503, "Google Wallet todav?a no est? configurado.")
    try:
...
def push_subscribe(
    ...
    if not settings.web_push_configured:
        raise HTTPException(503, "Las notificaciones todav?a no est?n configuradas.")
