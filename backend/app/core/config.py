...
class Settings(BaseSettings):
    app_name: str = "Orb?tica Loyalty API"
    app_env: str = "development"
    database_url: str = "sqlite:///./orbitica.db"

    jwt_secret: str = "dev-only-secret-that-is-at-least-32-chars-long"
    ...
    demo_business_name: str = "Barber?a Porras"
    ...
    def validate_runtime(self) -> None:
        ...
        if len(self.jwt_secret) < 32:
            raise RuntimeError("JWT_SECRET debe tener al menos 32 caracteres en producci?n.")
        if len(self.bff_shared_secret) < 32:
            raise RuntimeError("BFF_SHARED_SECRET debe tener al menos 32 caracteres en producci?n.")
        if self.apple_wallet_enabled:
            if not self.apple_wallet_updates_configured:
                raise RuntimeError("Apple Wallet est? habilitado pero faltan credenciales o configuraci?n.")
            if not self.public_api_url.startswith("https://"):
                raise RuntimeError("PUBLIC_API_URL debe usar HTTPS en producci?n para Apple Wallet.")
        if self.google_wallet_enabled and not self.google_wallet_configured:
            raise RuntimeError("Google Wallet est? habilitado pero faltan credenciales.")
        if self.web_push_enabled and not self.web_push_configured:
            raise RuntimeError("Web Push est? habilitado pero faltan credenciales VAPID.")
